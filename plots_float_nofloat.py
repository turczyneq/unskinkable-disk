...
def velocity(Q, m_disk, V):
    ...
    g = 9.81
    xi = 0.441
    return (xi * (rho**2 * Q**3 * g / (4 * np.pi * nu))**(0.5) - g * (m_disk - rho * V))/(rho*Q)
...
params = np.array(([[4, 11.7, 0.1, 2.77, 0.05],[5, 30.7, 0.1, 14.92, 0.05],[7,50.9, 0.2, 12.16, 0.05]]))
...
for pars in params:
    ...
    plt.errorbar(floating[step][:,0], floating[step][:,2], xerr=floating[step][:,1], yerr=floating[step][:,3], fmt='o', mfc='w', label=r"Disk floats")
    plt.errorbar(nofloat[step][:,0], nofloat[step][:,2], xerr=nofloat[step][:,1], yerr=nofloat[step][:,3], fmt='o', color='#d39400', label=r"Disk sinks")
